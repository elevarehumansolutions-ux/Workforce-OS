...
- **Error shape, consistent everywhere:** `{ "error": { "code": "VALIDATION_ERROR", "message": "..." } }`. Status codes used conventionally: 400 (validation), 401 (not authenticated), 403 (authenticated but not allowed, the department-check failures from `03_ARCHITECTURE.md`), 404, 409 (conflict, e.g. KPI weights not summing to 100).
