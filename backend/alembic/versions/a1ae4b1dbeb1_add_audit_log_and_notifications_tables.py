...
def upgrade() -> None:
    """Create audit_log and notifications tables with tenant-isolation RLS.

    Creates ``audit_log`` (organization_id, actor_user_id, action,
    entity_type/id, JSONB changes) and ``notifications`` (organization_id,
    recipient_user_id, category, title/body, optional link, read_at), each
    with supporting indexes, then enables row level security on both and
    adds a ``tenant_isolation`` policy scoping every row to
    ``app.current_org_id`` (no bootstrap exception — both only ever get
    written within an already-established org context).
    """
...
def downgrade() -> None:
    """Drop the tenant-isolation policies and the audit_log/notifications tables."""
